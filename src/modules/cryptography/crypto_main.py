...
class CryptoMainWindow(tk.Frame):
    ...
    def _adv_morse_ui(self):
        ...
        # Input area
        tk.Label(frame, text="Input (Text, Morse, or Audio File):", font=("Arial", 10), bg=Theme.get_color('primary'), fg=Theme.get_color('accent')).pack(anchor='w', padx=10)
        ...
        # File load button
        def load_file():
            file_path = filedialog.askopenfilename(title="Load file (text or audio)", filetypes=[("Text files", "*.txt"), ("WAV audio", "*.wav"), ("All files", "*.*")])
            if file_path:
                ext = os.path.splitext(file_path)[1].lower()
                if ext == '.wav':
                    self.morse_file_path = file_path
                    self.morse_input.delete('1.0', tk.END)
                    self.morse_input.insert('1.0', f"[Loaded audio file: {os.path.basename(file_path)}]")
                else:
                    try:
                        with open(file_path, 'r', encoding='utf-8') as file:
                            content = file.read()
        ...
        def to_text():
            ...
            self.morse_output.insert('1.0', result)
        def from_audio():
            # Always use the loaded .wav file path, never the input text
            if not self.morse_file_path:
                messagebox.showwarning("Warning", "No audio file loaded.")
                return
            morse, err = audio_to_morse(self.morse_file_path)
            self.morse_output.delete('1.0', tk.END)
            if err:
                self.morse_output.insert('1.0', err)
            else:
                self.morse_output.insert('1.0', morse if morse is not None else '')
                # Optionally, decode to text as well
                text = morse_to_text(morse)
                self.morse_output.insert(tk.END, f"\n\nDecoded Text: {text}")
        def clear():
            self.morse_input.delete('1.0', tk.END)
            self.morse_output.delete('1.0', tk.END)
            self.morse_file_path = None
        def copy_output():
            ...
            messagebox.showinfo("Copied", "Output copied to clipboard!")
        tk.Button(btn_frame, text="Text to Morse", font=("Arial", 10, "bold"), bg=Theme.get_color('accent'), fg=Theme.get_color('primary'), command=to_morse).pack(side='left', padx=5)
        tk.Button(btn_frame, text="Morse to Text", font=("Arial", 10), bg=Theme.get_color('secondary'), fg=Theme.get_color('accent'), command=to_text).pack(side='left', padx=5)
        tk.Button(btn_frame, text="Audio to Morse/Text", font=("Arial", 10), bg=Theme.get_color('secondary'), fg=Theme.get_color('accent'), command=from_audio).pack(side='left', padx=5)
        tk.Button(btn_frame, text="Clear", font=("Arial", 10), bg=Theme.get_color('secondary'), fg=Theme.get_color('accent'), command=clear).pack(side='left', padx=5)
        tk.Button(btn_frame, text="Copy Output", font=("Arial", 10), bg=Theme.get_color('secondary'), fg=Theme.get_color('accent'), command=copy_output).pack(side='left', padx=5)
        # Output area
        tk.Label(frame, text="Output:", font=("Arial", 10), bg=Theme.get_color('primary'), fg=Theme.get_color('accent')).pack(anchor='w', padx=10)
...
def morse_to_text(morse):
    ...
    for word in words:
        chars = word.split()
        decoded.append(''.join(MORSE_CODE_DICT_REVERSE.get(c, '?') for c in chars))
    return ' '.join(decoded)
...
def audio_to_morse(file_path):
    try:
        ...
        return None, f'WAV decode error: {e}'
    threshold = 0.2
    is_tone = np.abs(samples) > threshold
    import itertools
    groups = [(k, sum(1 for _ in g)) for k, g in itertools.groupby(is_tone)]
    lengths = [l for k, l in groups if k]
    if not lengths:
        return '', 'No Morse detected.'
    dot = min(lengths)
    dash = max(lengths)
    morse = ''
    for idx, (k, l) in enumerate(groups):
        if k:
            if l < (dot + dash) / 2:
                morse += '.'
            else:
