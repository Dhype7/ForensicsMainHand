...
class FileAnalyzerMainWindow(tk.Frame):
    ...
    def _run_file_breaker(self):
        ...
        if not wordlist:
            wordlist = filedialog.askopenfilename(title="Select Wordlist", filetypes=[("Wordlist Files", "*.txt *.lst *wordlist*"), ("All Files", "*.*")])
            if not wordlist:
                self.display_error("No wordlist selected.")
                return
            self.selected_wordlist_path = wordlist
        self.show_loading()
        def breaker():
            try:
                from .file_utils import FileAnalyzerUtils
