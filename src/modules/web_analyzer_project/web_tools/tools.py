...
def resolve_domain(domain: str) -> dict:
    try:
        ...
        try:
            ...
            result['IPv4'] = ', '.join([r.to_text() for r in a_records])
        except Exception:
            result['IPv4'] = ''
        # IPv6
        try:
            aaaa_records = dns.resolver.resolve(domain_only, 'AAAA')
            result['IPv6'] = ', '.join([r.to_text() for r in aaaa_records])
        except Exception:
            result['IPv6'] = ''
        # CNAME
        try:
            cname = dns.resolver.resolve(domain_only, 'CNAME')
            result['CNAME'] = ', '.join([str(r) for r in cname])
        except Exception:
            result['CNAME'] = ''
        # MX
        try:
            mx = dns.resolver.resolve(domain_only, 'MX')
            result['MX'] = ', '.join([str(r) for r in mx])
        except Exception:
            result['MX'] = ''
        return result
    except Exception as e:
